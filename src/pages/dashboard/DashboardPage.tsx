...
export default function DashboardPage() {
  ...
  const [parsedData, setParsedData] = useState<ParseTransactionResponse | null>(
    null,
  );
  const [categoryView, setCategoryView] = useState<"EXPENSE" | "INCOME">(
    "EXPENSE",
  );
  ...
  const [selectedTransferTx, setSelectedTransferTx] =
    useState<TransactionListDto | null>(null);
  ...
  }, [selectedAccountId, selYear, selMonth]);
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-4 animate-pulse">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-16 bg-gray-100 dark:bg-[#252523] rounded-xl"
            />
          ))}
        </div>
        ...
        <div className="px-1 py-2">
          ...
          {includedAccounts.length < accounts.length && (
            <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-1">
              ...
              excluida{accounts.length - includedAccounts.length > 1 ? "s" : ""}
            </p>
          )}
        </div>
      )}
              ...
              (() => {
                ...
                return (
                  ...
                  >
                    ...
                    <X
                      size={11}
                      className="text-gray-400 dark:text-gray-500 ml-0.5"
                    />
                  </button>
                );
              })()}
